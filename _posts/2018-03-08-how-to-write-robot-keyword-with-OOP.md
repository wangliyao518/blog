...
这段代码其实还有第二个问题, 就是维护的问题, 新增加一种要抓log类型时, 或者是start_*log的接口函数修改之时都面临着我们要修改这段, 有修改就意外这都需要经历过测试才能保证这块代码的质量, 新增的log也要再次测试之前的syslog, ttitrace, aaMessageTtrace这显然也不是合理.
